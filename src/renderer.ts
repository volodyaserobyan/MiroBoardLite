...
import { StickyNote } from './types.js';
...
export class Renderer {
   ...
   */
  render(state: AppState, editingNoteId: number | null): void {
    const w = window.innerWidth;
    const h = window.innerHeight;
  ...
  private renderInteractionLayer(state: AppState, w: number, h: number): void {
    ...
    for (const obj of visible) {
      if (obj.selected) this.drawSelectionRing(ctx, state, obj);
    }
  }
  ...
  private drawMinimap(
    ...
    if (state.scene.length === 0) return;
  ...
  private createCanvas(container: HTMLElement, zIndex: number): HTMLCanvasElement {
  ...
  private getVisibleSorted(state: AppState, w: number, h: number): StickyNote[] {
