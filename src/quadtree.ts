...
import { Rect, StickyNote } from './types.js';
...
export class QuadTree {
  ...
  private bounds: Rect;
  private objects: StickyNote[] = [];
  private nodes: QuadTree[] = [];

  constructor(bounds: Rect, level = 0) {
    this.bounds = bounds;
    this.level = level;
  }
  ...
  query(range: Rect): StickyNote[] {
    const found: StickyNote[] = [];
  ...
  queryUnique(range: Rect): StickyNote[] {
  ...
  private getIndices(rect: Rect): number[] {
...
function objToRect(obj: StickyNote): Rect {
